...
// unsatisfiable and churn the lock into persistent [Timeout] errors. The
// per-model fields come from WEEKLY_MODEL_USAGE_BUCKETS so a bucket added
// upstream is served rather than filtered out.
...
const CachedUsageDataSchema = z.object({
    ...
    // Declared so the schema stays total over UsageData even though the
    // Anthropic fetcher never produces the monthly pool (Kimi caches it
    // separately); an absent key here would make UsageDataField unindexable.
