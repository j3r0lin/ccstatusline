...
describe('jsonl transcript metrics', () => {
    ...
    it('reports post-compaction context length from the boundary marker before the next turn', async () => {
        ...
        expect(metrics).toEqual({
            ...
            totalTokens: 235100,
            contextLength: 18000,
            lastCompletionMs: new Date('2026-01-01T10:00:00.000Z').getTime()
        });
    });
    ...
    it('uses the first turn after a compaction boundary instead of its postTokens estimate', async () => {
        ...
        expect(metrics).toEqual({
            ...
            totalTokens: 252850,
            contextLength: 17700,
            lastCompletionMs: new Date('2026-01-01T10:06:00.000Z').getTime()
        });
    });
    ...
    it('aggregates token metrics across many usage records', async () => {
        ...
        expect(metrics).toEqual({
            ...
            contextLength: 2 + 4 + 1,
            lastCompletionMs: new Date('2026-01-01T10:41:39.000Z').getTime()
        });
    }, 30000);
    ...
    it('collects configured transcript metrics in one combined analysis', async () => {
        ...
        expect(analysis).toEqual({
            tokenMetrics: {
                ...
                totalTokens: 10,
                contextLength: 5,
                lastCompletionMs: new Date('2026-01-01T10:01:00.000Z').getTime()
            },
            sessionDuration: '4m',
